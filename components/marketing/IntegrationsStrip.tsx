  ...
  return (
    <div>
      <ScrollReveal>
        ...
        <h2 className="mt-6 max-w-[30ch] font-sans text-[clamp(2rem,3.8vw,3.25rem)] font-semibold leading-[1.05] tracking-[-0.02em] text-[var(--foreground)]">
      ...
      <ScrollRevealStagger className="mt-16 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-6 sm:gap-6">
        {integrations.map(({ icon: Icon, label }) => (
          <li key={label}>
            <div className="flex flex-col items-center gap-4 rounded-none border border-[var(--border-subtle)] bg-[var(--bg)] p-6 text-center transition-colors duration-300 hover:border-[rgba(124,58,237,0.35)]">
              <Icon
