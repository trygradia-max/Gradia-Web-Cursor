...
> This brief is downstream of the single source of truth:
> [`../_docs/WHAT_GRADIA_DOES.md`](../_docs/WHAT_GRADIA_DOES.md).
> **The rule (from that doc): if a claim isn't in `WHAT_GRADIA_DOES.md` §5, we don't make it.**
> Where this brief and that doc ever disagree, **that doc wins** — update this file.
...
- Quotes and books **over the phone**
- Follows up with every lead by text + email
- Revives old leads you forgot about
- **Whisper:** speak an instruction → Gradia stages the work
- **Agentic mode:** describe a workflow → Gradia runs it
- You approve everything before it sends
- Everything in one CRM + calendar
- $20/month, less than one detail a month
- Built FOR detailers

## 5. 🚫 NEVER say (forbidden — from §3; the code contains some of these, but they are NOT shipping in the MVP)

- ❌ **"7 agents"** — it's **two** (voice + chat). The old 7-agent cast is retired.
- ❌ **Collect money / invoicing / payments / "get paid"** — billing is hidden for MVP.
- ❌ **Instagram or Facebook DMs** — Meta channels exist in code but are hidden for MVP.
- ❌ **Photo-based quoting / "quote every car from a photo"** — only phone quotes.
- ❌ **Social posting / "we post for you"** — not in MVP.
- ❌ **Auto-send / "hands-off" / "fully autonomous" / "set it and forget it"** — approval is
  ALWAYS required. Never imply the AI sends without the owner.

> ⚠️ **Note for coding agents:** the platform repo *does* contain Instagram/Facebook drafters,
> Stripe/payment code, and a BI agent. Their existence in code does **not** make them
> marketing-safe. Claim only what's in §4.

## 6. Before/after — the current hero is STALE, fix it

**Current (violates the brief — "7 agents" + "collect your money"):**

> Your entire front office. One app. Zero missed jobs.
...
- Speak to a **detailer**, not a "business." Concrete, blue-collar-respecting, no SaaS jargon
  ("synergy," "platform," "solution"). Short sentences.
- Sell the outcome (never miss a call, fill the calendar, bring back customers), then the
  mechanism (one brain, you approve everything).
- Match the product's own voice: Gradia always speaks as **"we/us"** for the shop, never as a
  third-party bot.

## 8. Open items (confirm before publishing)

- **Is $20/month cleared for public display** on the landing page, or held for waitlist reveal?
- Apply this voice consistently across **every section, the waitlist form, and meta/OG tags.**

---

_Source of truth: `../_docs/WHAT_GRADIA_DOES.md` (updated 2026-06-04). This brief is a
marketing-facing summary of it — when in doubt, open that file._
